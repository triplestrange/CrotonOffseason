...
public class SwerveTrajectory extends Trajectory {
	...
	public static SwerveTrajectory generate(Trajectory.Config config, SwerveWaypoint[] waypoints, double angularVelocity, double angularAcc) {
		...
		trajectory.gyroProfile[0] = new TrapezoidProfile(trajectory.waypoints[0].rotation, trajectory.waypoints[1].rotation, 0, 0, trajectory.angularVelocity, trajectory.angularAcc);
		for (int i = 1; i < waypoints.length - 1; i++) {
			double startPos = trajectory.gyroProfile[i-1].currentP(trajectory.waypointTime[i]-trajectory.waypointTime[i-1]);
			double startV = trajectory.gyroProfile[i-1].currentV(trajectory.waypointTime[i]-trajectory.waypointTime[i-1]);
			double endPos = trajectory.waypoints[i+1].rotation;
			trajectory.gyroProfile[i] = new TrapezoidProfile(startPos, endPos, startV, 0, trajectory.angularVelocity, trajectory.angularAcc);
		}
		return trajectory;
	}
